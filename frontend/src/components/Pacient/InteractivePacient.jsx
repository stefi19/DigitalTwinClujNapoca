...
export default function InteractivePacient() {
  ...
  const progressStateRef = useRef({ raf: null, startTime: null, duration: null, coords: null, segLengths: null, total: null });
  ...
  const startDispatch = useCallback(async () => {
    ...
    totalDurationRef.current = data.routes[0].duration || null;
  ...
  function startProgressForRoute(routeGeom, ambObj) {
    ...
    if (!duration) {
      // fallback: use speed_kmh if available
  ...
  // reset any previous animation and ETA
  ...
  setEta(null);
    ...
    state.total = total;
    state.duration = duration * 1000; // ms
  ...
  function handleSelectAmb(id) {
    ...
    if (amb.route) {
      startProgressForRoute(amb.route, amb);
    } else {
        ...
        (async () => {
          try {
            ...
            if (data && data.routes && data.routes[0] && data.routes[0].geometry) {
              ...
              startProgressForRoute(geom, ambWithEta);
            } else {
             ...
             (showOnlyEnroute ? ambulances.filter(a => a.status === 'enroute') : ambulances)
                ...
                try {
                  if (a.lat != null && a.lon != null) {
                    const d = Math.round(haversine([Number(a.lon),Number(a.lat)],[patientLng,patientLat]));
                    distStr = d >= 1000 ? (d/1000).toFixed(1)+' km' : d+' m';
                    if (a.eta) {
                      const rem = Math.max(0, (new Date(a.eta).getTime() - Date.now())/1000);
                      etaStr = rem <= 0 ? 'Arrived' : (rem < 60 ? '<1m' : Math.ceil(rem/60) + 'm');
                    } else if (d && a.speed_kmh) {
                      const speedMs = Number(a.speed_kmh) * 1000/3600;
                      const rem = Math.max(0, d / Math.max(0.1, speedMs));
                      etaStr = rem < 60 ? '<1m' : Math.ceil(rem/60) + 'm';
                    }
                  }
                } catch(e){}
                return (
